...
import { showFromMsg, mealInputMessage, caloriesInputMessage, saveMealMsg, deleteMeal, editMealMsg } from './Update'
...
function buttonSet(dispatch){
    return div([
        ...
        button({
            className: 'f3 pv2 ph3 bg-blue white bn mr2 dim',
            onclick: ()=> dispatch(showFromMsg(false))
        },
        'Cancel'
        )
    ])
...
function formView(dispatch, model){
    ...
    if(showForm){
        return form({className:'w-100 mv2', onsubmit: e=>{ 
            e.preventDefault()
            dispatch(saveMealMsg)
        } },[
            fieldSet('Meal', description,(e)=> dispatch(mealInputMessage(e.target.value))),
            fieldSet('Calorie', calories || '',(e)=> dispatch(caloriesInputMessage(e.target.value))),
            buttonSet(dispatch)
        ])
    }
    return button({className:"f3 pv2 ph3 bg-blue white bn" , onclick: ()=> dispatch(showFromMsg(true))},'Add Meal')
}
